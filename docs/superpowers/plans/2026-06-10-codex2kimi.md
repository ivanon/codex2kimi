...
| `deploy/com.codex2kimi.proxy.plist` | LaunchAgent plist 模板 |
...
- Create: `.gitignore`
- Create: `src/index.ts`（占位入口，仅用于验证工具链）
...
```bash
git add package.json package-lock.json tsconfig.json vitest.config.ts .gitignore src/index.ts tests/smoke.test.ts
...
export function resolveConfig(
  ...
  return {
    ...
    port: portRaw !== undefined ? Number(portRaw) : DEFAULTS.port,
...
export function translateError(
  ...
  return {
    ...
    body: {
      error: {
        type: TYPE_MAP[err.type] ?? "api_error",
        message: err.message,
        code: null,
      },
    },
...
test("error event maps to response.failed", () => {
  const t = new StreamTranslator(OPTS);
  t.handle(START);
    ...
    switch (event.type) {
      ...
      case "error":
        return [this.evt("response.failed", {
          response: { ...this.skeleton("failed"), error: { type: event.error.type, message: event.error.message } },
        })];
      default:
        return [];
    }
...
> `translateResponse(upstream.body as never, ...)`：上游 JSON 已确认是 Anthropic 响应；`as never` 仅为绕过 `unknown`，运行时形态由 Task 7 的转换函数兜底。`pumpStream` 的 catch 用 `translator.incomplete()` 对应 spec「中途断流 → response.incomplete」。
...
- Create: `tests/replay.test.ts`（离线回放，普通单测）
- Create: `vitest.integration.config.ts`
- Create: `scripts/record-fixture.ts`（联调录制脚本）
- Create: `tests/e2e.integration.test.ts`（env 门控）
...
test("replays recorded anthropic text SSE into valid Responses stream", async () => {
  const fixture = readFileSync(join(__dirname, "../fixtures/anthropic/stream/text.sse"), "utf8");
  const translator = new StreamTranslator({ model: "gpt-5-codex", createdAt: 1000, parallelToolCalls: true });
  const out = await drain(pumpStream(streamFromText(fixture), translator));
...
Expected: 1 passed（`pumpStream`/`StreamTranslator` 已在 Task 10/12 实现，本测试一次性通过即可）。
...
it("real Kimi: non-stream text round-trip returns completed", async () => {
  ...
  const res = await app.request("/v1/responses", {
    ...
    body: JSON.stringify({
      model: "gpt-5-codex",
      stream: false,
      input: [{ type: "message", role: "user", content: [{ type: "input_text", text: "Reply with the single word: ok" }] }],
    }),
  });
  expect(res.status).toBe(200);
